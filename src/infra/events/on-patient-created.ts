...
function buildPatientEmail(email: string, password: string, expiresAt: Date): string {
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
          ...
          <tr>
            <td style="background:#fff;border-radius:0 0 12px 12px;padding:36px 40px;box-shadow:0 4px 16px rgba(0,0,0,0.06);">
              <h2 style="margin:0 0 8px;color:#0f172a;font-size:22px;">Bem-vindo ao Cliniker!</h2>
              <p style="margin:0 0 24px;color:#475569;font-size:15px;line-height:1.6;">
                Seu cadastro como <strong>paciente</strong> foi realizado. Use as credenciais abaixo para acessar o sistema.
              </p>
...
export class OnPatientCreated {
  private readonly logger = new Logger(OnPatientCreated.name)
  ...
  async handle(event: PatientCreatedEvent) {
    this.logger.log(`Handler OnPatientCreated: patientId=${event.patientId} clinicId=${event.clinicId}`)
    ...
    try {
      ...
      this.logger.log(`Email de boas-vindas enfileirado: patientId=${event.patientId}`)
    } catch (error) {
      this.logger.error(`Erro ao enfileirar email: patientId=${event.patientId}`, error)
      throw error;
    }
  }
}
