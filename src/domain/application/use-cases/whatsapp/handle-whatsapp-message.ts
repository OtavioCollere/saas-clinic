...
export class HandleWhatsAppMessageUseCase {
  ...
  async execute({ franchiseId, fromPhone, message }: HandleWhatsAppMessageUseCaseRequest): Promise<void> {
    ...
    if (conversation.step.isAwaitingCpf()) {
      await this.handleAwaitingCpf(conversation, franchise.clinicId.toString(), message, credentials)
      return
    }

    if (conversation.step.isAwaitingProfessional()) {
      await this.handleAwaitingProfessional(conversation, franchiseId, message, credentials)
      return
    }

    if (conversation.step.isAwaitingDate()) {
      await this.handleAwaitingDate(conversation, message, credentials)
      return
    }

    if (conversation.step.isAwaitingTime()) {
      await this.handleAwaitingTime(conversation, message, credentials)
      return
    }

    if (conversation.step.isAwaitingConfirmation()) {
      await this.handleAwaitingConfirmation(conversation, franchiseId, message, credentials)
    }
  }

  private async handleAwaitingCpf(
    conversation: WhatsAppConversation,
    clinicId: string,
    message: string,
    credentials: ZApiCredentials | undefined,
  ) {
  ...
  private async handleAwaitingProfessional(
    conversation: WhatsAppConversation,
    franchiseId: string,
    message: string,
    credentials: ZApiCredentials | undefined,
  ...
  private async handleAwaitingTime(
    conversation: WhatsAppConversation,
    message: string,
    credentials: ZApiCredentials | undefined,
  ) {
